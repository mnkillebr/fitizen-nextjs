...
import { AUTH_SESSION_MAX_AGE, MAGIC_LINK_MAX_AGE, NEW_LOG_MAX_AGE } from '@/lib/magicNumbers'
...
export async function newProgramLog({ programLogId, workoutName }: {programLogId: string, workoutName: string}) {
  const cookieStore = await cookies()
  const currentDate = new Date()
  const expiresAt = new Date(currentDate.getTime() + NEW_LOG_MAX_AGE)
...
export async function newWorkoutLog({ workoutLogId, workoutName }: {workoutLogId: string, workoutName: string}) {
  const cookieStore = await cookies()
  const currentDate = new Date()
  const expiresAt = new Date(currentDate.getTime() + NEW_LOG_MAX_AGE)

  cookieStore.set('fitizen__new_workout_log', JSON.stringify({ workoutLogId, workoutName }), {
    httpOnly: true,
    secure: true,
    sameSite: 'lax',
    path: '/',
